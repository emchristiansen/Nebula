...
case class NNClassifier[A](
  distances: List[Tuple2[Int, NNClassifier.ImageDistance]],
  trainData: List[Tuple2[A, BufferedImage]]) {
    ...
    distances match {
      ...
      case (numNeighbors, distance) :: remainingDistances => {
        ...
        val remainingCandidates =
          distancesAndLabels.sortBy(_._1).take(numNeighbors).map(_._2)
