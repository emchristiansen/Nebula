...
//    println(scale)
//    println(leftHist.map(_.keys.toList.head))
//    println(rightHist.map(_.keys.toList.head))
    
//    println(cost.map(_.toList).toList)
//    println(optimalAssignment)
...
//    println(optimalCost)
//    
//    println(estimateL1Matching(scale, dimension, leftHist, rightHist))
  }
}
