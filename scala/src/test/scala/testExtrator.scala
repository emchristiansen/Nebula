...
class TestExtractor extends FunSuite {
  test("uniformRank") {
    val pixels = RawDescriptor(IndexedSeq(2, 2, 3, 3, 3))
    val uniformRank = ExtractorParameterized.uniformRank(pixels)
    assert(uniformRank === RawDescriptor(IndexedSeq(1, 1, 3, 3, 3)))
  }
  ...
  test("BRISK") {
    ...
    val extractor = BRISKRawExtractor(true, true)
